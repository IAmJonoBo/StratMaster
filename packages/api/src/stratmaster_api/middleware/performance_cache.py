...
from .advanced_cache import get_multi_tier_cache, is_response_cache_v2_enabled, is_edge_cache_hints_enabled
...
try:
    ...
    CACHE_HITS = Counter('cache_hits_total', 'Total cache hits', ['endpoint', 'cache_level'])
    ...
    RESPONSE_SIZE_BYTES = Histogram('response_size_bytes', 'Response size in bytes', ['endpoint'])
...
class PerformanceProfilingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # Record metrics
            if PROMETHEUS_AVAILABLE:
                ENDPOINT_LATENCY.labels(endpoint=endpoint, method=request.method).observe(process_time / 1000)
