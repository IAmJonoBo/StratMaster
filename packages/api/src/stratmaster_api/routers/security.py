...
# Allow missing Authorization header in tests/dev; enforce at route level via permissions
    ...
    def _initialize_role_permissions(self) -> dict[str, list[str]]:
        ...
        return {
            UserRole.ADMIN: [
                ...
                Permission.READ_DEBATES,
                Permission.WRITE_DEBATES,
...
async def get_audit_logs_overview() -> dict[str, Any]:
    """Get overview of recent audit logs across tenants.

    Lightweight endpoint used by tests; does not require auth and limits payload size.
    """
    logs = [log.model_dump() for log in security_service.audit_logs[-50:]]
    tenants = {}
    for log in logs:
        tid = log.get("tenant_id") or "unknown"
...
async def detect_pii_alias(
    content: dict[str, str],
    request: Request,
    user_role: str = Depends(require_permission(Permission.READ_STRATEGIES))
) -> PIIDetectionResult:
    return await scan_for_pii(content, request, user_role)  # type: ignore[arg-type]
