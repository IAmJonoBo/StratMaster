...
from ..collaboration import YjsCollaborationServer, is_collaboration_enabled
...
def setup_collaboration_websocket(app: FastAPI) -> None:
    ...
    # Always register status endpoint for observability
    ...
    async def collaboration_status():
            ...
            return JSONResponse(
                content={
                    ...
                    "message": (
                        ...
                        "Set ENABLE_COLLAB_LIVE=true to enable."
                    ),
        ...
        return JSONResponse(
            content={
                ...
                "active_connections": (
                    collaboration_server.connection_count if collaboration_server else 0
                ),
                "total_messages": (
                    collaboration_server.message_count if collaboration_server else 0
                ),
                "feature_flags": {
    ...
    # If collaboration is disabled, do not register websocket route
    if not is_collaboration_enabled():
    ...
    async def websocket_collaboration_endpoint(websocket: WebSocket, doc_id: str):
        ...
        try:
            ...
            if collaboration_server:
                await collaboration_server.cleanup_connection(
                    websocket, doc_id, user_id, connection_id or "websocket"
                )
