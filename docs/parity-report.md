...
| **Helm Charts** | [Deployment Guide](how-to/deployment.md) | [`helm/`](../helm/) | ✅ Linted (0 failures) |
...
1. **🔥 High Priority** - Implement missing experiment and forecast endpoints
2. **📋 Medium Priority** - Wire up MCP server connections
3. **📋 Medium Priority** - Complete constitutional ML components
4. **🔧 Low Priority** - Clean remaining sprint language from archived docs
...
- [x] `helm lint` passes for all charts (stratmaster-api, research-mcp)
- [x] `scripts/smoke_api.py` exists and succeeds
